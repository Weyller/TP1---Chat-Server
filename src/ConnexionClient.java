	...
	public void run() {
		...
		try {
	       ...
	       // conversation.clients.add(nom);
	        ...
	        if(choix.equals("j")){
	        	//*a faire
	        	//envoyer les joueur vers un thread salle d'attente et d�s qu'il y a un bourreau et un temoin ils commencent la partie
	        	
	        	System.out.println("Nb de clients = " + conversation.getClients().size());
	        	
	        	if(role == "bourreau"){
	        		
	        		out.println("Vous etes le " + role);
	        		out.println("");
	        		out.flush();
	        		
	        		listeSalleAttente.add(nom);
	     	        
	        		SalleAttente attente = new SalleAttente(socket, conversation, nom);
	        		attente.run();
